...
# Please mind that the model objects are not provided (the posteriors, just for the two main models,
# are ~700 MB each, for all models, we are talking about tens of Gigabytes), but you have the data 
# and the script. All you need to do is to sample (and save) the posteriors on your own. 
...
post_ATD_F <- readRDS("post_F_ATD_22_08_25_long.RDS") # This is a model with A / T / D at once

str(post_ATD_F)

# MALES

post_ATD_M <- readRDS("post_M_ATD_22_08_25_long.RDS") # This is a model with A / T / D at once
...
tiff("F_ATD_model_LEVELS_REV.tiff",width=34,height=14,units="cm",res=600,compression="lzw")
...
tiff("F_ATD_model_LEVELS_NOLABELS_REV.tiff",width=30,height=14,units="cm",res=600,compression="lzw")
layout(matrix(1:3,byrow=F,nrow=1), widths=c(1.8,1.2,1.2),heights=c(1))
...
tiff("Correlations_Heatmap_ATD_Females_Attr_wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AF
dev.off()

tiff("Correlations_Heatmap_ATD_Females_Tru_wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AT
dev.off()

tiff("Correlations_Heatmap_ATD_Females_Dom_wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AD
dev.off()


#---------
# QUESTION: Which is the average correlation between raters from the two groups: 
str(post_ATD_F$Rho_FSMUi_A)

differences <- c((post_ATD_F$Rho_FSMUi_A[,1,3] - post_ATD_F$Rho_FSMUi_A[,2,4]),
                 (post_ATD_F$Rho_FSMUi_A[,1,5] - post_ATD_F$Rho_FSMUi_A[,2,6]),
                 (post_ATD_F$Rho_FSMUi_A[,1,7] - post_ATD_F$Rho_FSMUi_A[,2,8]), 
                 (post_ATD_F$Rho_FSMUi_A[,1,9] - post_ATD_F$Rho_FSMUi_A[,2,10]), 
                 (post_ATD_F$Rho_FSMUi_A[,1,11] - post_ATD_F$Rho_FSMUi_A[,2,12]), 
                 (post_ATD_F$Rho_FSMUi_A[,3,5] - post_ATD_F$Rho_FSMUi_A[,4,6]), 
                 (post_ATD_F$Rho_FSMUi_A[,3,7] - post_ATD_F$Rho_FSMUi_A[,4,8]), 
                 (post_ATD_F$Rho_FSMUi_A[,3,9] - post_ATD_F$Rho_FSMUi_A[,4,10]), 
                 (post_ATD_F$Rho_FSMUi_A[,3,11] - post_ATD_F$Rho_FSMUi_A[,4,12]), 
                 (post_ATD_F$Rho_FSMUi_A[,5,7] - post_ATD_F$Rho_FSMUi_A[,6,8]), 
                 (post_ATD_F$Rho_FSMUi_A[,5,9] - post_ATD_F$Rho_FSMUi_A[,6,10]), 
                 (post_ATD_F$Rho_FSMUi_A[,5,11] - post_ATD_F$Rho_FSMUi_A[,6,12]), 
                 (post_ATD_F$Rho_FSMUi_A[,7,9] - post_ATD_F$Rho_FSMUi_A[,8,10]), 
                 (post_ATD_F$Rho_FSMUi_A[,7,11] - post_ATD_F$Rho_FSMUi_A[,8,12]), 
                 (post_ATD_F$Rho_FSMUi_A[,9,11] - post_ATD_F$Rho_FSMUi_A[,10,12])
) 

mean(differences)
PI(differences, prob=0.95)


# A tohle bude to samý: 
cultures <- c(1,3,5,7,9,11)

subset_Rho <- post_ATD_F$Rho_FSMUi_A[,cultures,cultures]

# Remove self-correlations (diagonal elements) by setting them to NA
for (i in seq_along(cultures)) {
  subset_Rho[, i, i] <- NA  # Set diagonal to NA
}

subset_Rho <- as.vector(subset_Rho)
subset_Rho <- subset_Rho[(!is.na(subset_Rho))]
mean(subset_Rho)


cultures <- c(2,4,6,8,10,12)

subset_Rho2 <- post_ATD_F$Rho_FSMUi_A[,cultures,cultures]

# Remove self-correlations (diagonal elements) by setting them to NA
for (i in seq_along(cultures)) {
  subset_Rho2[, i, i] <- NA  # Set diagonal to NA
}

subset_Rho2 <- as.vector(subset_Rho2)
subset_Rho2 <- subset_Rho2[(!is.na(subset_Rho2))]
mean(subset_Rho2)

corcomp <- subset_Rho - subset_Rho2
mean(corcomp)
PI(corcomp,prob=0.95)
# Ne úplně - ale skoro 
...
tiff("F_ATD_GMM_25_08.tiff",width=35,height=30,units="cm",res=600,compression="lzw")
...
tiff("F_ATD_GMM_25_08_no_COEFS.tiff",width=30,height=33,units="cm",res=600,compression="lzw")
...
# MEN 
# MEN
# MEN (whatever is after line 3000 - this must be men...)
...
tiff("M_ATD_model_LEVELS_REV.tiff",width=34,height=14,units="cm",res=600,compression="lzw")
layout(matrix(1:3,byrow=F,nrow=1), widths=c(1.8,1.2,1.2),heights=c(1))
...
tiff("M_ATD_model_LEVELS_NOLABELS_REV.tiff",width=30,height=14,units="cm",res=600,compression="lzw")
layout(matrix(1:3,byrow=F,nrow=1), widths=c(1.8,1.2,1.2),heights=c(1))
...
tiff("Correlations_Heatmap_ATD_Males_Attr_wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AF
dev.off()

tiff("Correlations_Heatmap_ATD_Males_Tru_wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AT
dev.off()

tiff("Correlations_Heatmap_ATD_Males_Dom_Wide_REV.tiff",width=57,height=25,units="cm",res=600,compression="lzw")
AD
dev.off()
...
differences <- c((post_ATD_M$Rho_FSMUi_A[,1,3] - post_ATD_M$Rho_FSMUi_A[,2,4]),
                 (post_ATD_M$Rho_FSMUi_A[,1,5] - post_ATD_M$Rho_FSMUi_A[,2,6]),
                 (post_ATD_M$Rho_FSMUi_A[,1,7] - post_ATD_M$Rho_FSMUi_A[,2,8]), 
                 (post_ATD_M$Rho_FSMUi_A[,1,9] - post_ATD_M$Rho_FSMUi_A[,2,10]), 
                 (post_ATD_M$Rho_FSMUi_A[,1,11] - post_ATD_M$Rho_FSMUi_A[,2,12]), 
                 (post_ATD_M$Rho_FSMUi_A[,3,5] - post_ATD_M$Rho_FSMUi_A[,4,6]), 
                 (post_ATD_M$Rho_FSMUi_A[,3,7] - post_ATD_M$Rho_FSMUi_A[,4,8]), 
                 (post_ATD_M$Rho_FSMUi_A[,3,9] - post_ATD_M$Rho_FSMUi_A[,4,10]), 
                 (post_ATD_M$Rho_FSMUi_A[,3,11] - post_ATD_M$Rho_FSMUi_A[,4,12]), 
                 (post_ATD_M$Rho_FSMUi_A[,5,7] - post_ATD_M$Rho_FSMUi_A[,6,8]), 
                 (post_ATD_M$Rho_FSMUi_A[,5,9] - post_ATD_M$Rho_FSMUi_A[,6,10]), 
                 (post_ATD_M$Rho_FSMUi_A[,5,11] - post_ATD_M$Rho_FSMUi_A[,6,12]), 
                 (post_ATD_M$Rho_FSMUi_A[,7,9] - post_ATD_M$Rho_FSMUi_A[,8,10]), 
                 (post_ATD_M$Rho_FSMUi_A[,7,11] - post_ATD_M$Rho_FSMUi_A[,8,12]), 
                 (post_ATD_M$Rho_FSMUi_A[,9,11] - post_ATD_M$Rho_FSMUi_A[,10,12])
) 
...
subset_Rho <- post_ATD_M$Rho_FSMUi_A[,cultures,cultures]
...
subset_Rho2 <- post_ATD_M$Rho_FSMUi_A[,cultures,cultures]
...
tiff("M_ATD_GMM_REV.tiff",width=35,height=30,units="cm",res=600,compression="lzw")
...
tiff("M_ATD_GMM_No_Labels_REV.tiff",width=33,height=30,units="cm",res=600,compression="lzw")
layout(matrix(1:3,byrow=F,nrow=1), widths=c(1.1,0.7,0.7),heights=c(1))
