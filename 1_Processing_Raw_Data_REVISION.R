# NOTE: We do not provide the raw data since they are not required for the analyses, 
# interested reader is suggested to contact the authors at vfiala@umk.pl
...
#.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
# The new version - we still create the Ad Hoc medal and punished scores, 
# but
# 1) We will check for underlying factor later on
# 2) Work with the extended Colombian Data.
#.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
...
COL_COL1 <- read.csv2("Colombia_19_08_25_tests_excluded.csv",T) # Single table - contains both the old ratings and the new ones!
...
COL_COL_P_ID$lengths <- tapply(COL_COL1$Rec_Session_Id, COL_COL1$Exp_Subject_Id, length)
...
# MARITAL STATUS: 
COL_COL_P_ID$MariStat <- tapply(COL_COL1$Marital_status_F1,COL_COL1$Exp_Subject_Id,mean, na.rm=T)
...
# Are there some complete non-users: 
summary(as.factor(COL_COL1$Use_SNS_NO)) # All 32 to respond the survey so far are users... 
...
COL_COL_P_ID$WhattsAPP_AP <- tapply(COL_COL1$AP_WhatsApp,COL_COL1$Exp_Subject_Id, mean, na.rm=T)
...
# Third one - which one (NAME): 
COL_COL1$AP_Input.socal.media.3 <- ifelse(COL_COL1$AP_Input.socal.media.3=="",NA,COL_COL1$AP_Input.socal.media.3)
COL_COL_P_ID$SELF_AP_NAME_3 <- tapply(COL_COL1$AP_Input.socal.media.3, COL_COL1$Exp_Subject_Id, get_word2)
...
# WeChat = How_Frequently_WECHAT
COL_COL_P_ID$WhatsApp_Freq <- tapply(COL_COL1$How_Frequently_WECHAT,COL_COL1$Exp_Subject_Id, mean, na.rm=T) # WE chat is WhatsAPP!!
...
# NAME_1: TIME_Input social media_1
COL_COL1$TIME_Input.social.media_1 <- ifelse(COL_COL1$TIME_Input.social.media_1=="",NA,COL_COL1$TIME_Input.social.media_1)
COL_COL_P_ID$Time_SELFREP_Name1 <- tapply(COL_COL1$TIME_Input.social.media_1, COL_COL1$Exp_Subject_Id, get_word2)
...
# NAME_2: TIME_Input social media_2
COL_COL1$TIME_Input.social.media.2 <- ifelse(COL_COL1$TIME_Input.social.media.2=="",NA,COL_COL1$TIME_Input.social.media.2)
COL_COL_P_ID$Time_SELFREP_Name2 <- tapply(COL_COL1$TIME_Input.social.media.2, COL_COL1$Exp_Subject_Id, get_word2)

# Frequency_2: TIME_2nd self reported SNS
COL_COL_P_ID$Time_SELFREP_Freq2 <- tapply(COL_COL1$TIME_2nd.self.reported.SNS,COL_COL1$Exp_Subject_Id, mean, na.rm=T)
...
# NAME_3: TIME_Input social media_3
COL_COL1$TIME_Input.social.media_3 <- ifelse(COL_COL1$TIME_Input.social.media_3=="",NA,COL_COL1$TIME_Input.social.media_3)
COL_COL_P_ID$Time_SELFREP_Name3 <- tapply(COL_COL1$TIME_Input.social.media_3, COL_COL1$Exp_Subject_Id, get_word2)

# Frequency_3: TIME_3nd self reported SNS
COL_COL_P_ID$Time_SELFREP_Freq3 <- tapply(COL_COL1$TIME_3nd.self.reported.SNS,COL_COL1$Exp_Subject_Id, mean, na.rm=T)
...
names(COLRAT_List_FACES)[names(COLRAT_List_FACES) == 'Exp_Subject_Id'] <- "COL_COL_P_ID"
...
OLDEMOG$Cultur <- rep(c("AUSNZ","COL","CZ","RSA","TUR","VN"),times=c(53,32,152,47,79,72))
...
write.csv(d, file="Whole_Dataset_long_GMM_INCLUDED_SIDESCENTRE_INCLUDED_SOCBACK_TRAVELABROAD_INCL_19_08_25.csv") 
write.csv2(d, file="1Whole_Dataset_long_GMM_INCLUDED_SIDESCENTRE_INCLUDED_SOCBACK_TRAVELABROAD_INCL_19_08_25.csv") 
# This time, it includes a variable that tells 
# This time, it includes a variable that tells 
# whether the rater should be kept - if she's close to the median in SMU frequency/intensity, she should be exluded...
#...but only in ONE analysis.
...
Cor_A_T_1 <- data.frame(
  Attr_CZ_O = unlist(tapply(CZ_Orig$Atraktivita, CZ_Orig$FuFa_ID, mean)),
  Trustw_CZ_O = unlist(tapply(CZ_Orig$Duveryhodnost, CZ_Orig$FuFa_ID, mean))
)

Cor_A_T_2 <- data.frame(
  Attr_CZ_N = unlist(tapply(CZ_INTVAR$Atr, CZ_INTVAR$Face_ID, mean)),
  Trustw_CZ_N = unlist(tapply(CZ_INTVAR$Tru, CZ_INTVAR$Face_ID, mean))
)

Cor_A_T_3 <- data.frame(
  Attr_RSA = unlist(tapply(RSA_INTVAR$Atr, RSA_INTVAR$Face_ID, mean)),
  Trustw_RSA = unlist(tapply(RSA_INTVAR$Tru, RSA_INTVAR$Face_ID, mean))
)

Cor_A_T_4 <- data.frame(
  Attr_AUS = unlist(tapply(AUS_INTVAR$Atr, AUS_INTVAR$Face_ID, mean)),
  Trustw_AUS = unlist(tapply(AUS_INTVAR$Tru, AUS_INTVAR$Face_ID, mean))
)

Cor_A_T_5 <- data.frame(
  Attr_VN = unlist(tapply(VN_INTVAR$Atr, VN_INTVAR$Face_ID, mean)),
  Trustw_VN = unlist(tapply(VN_INTVAR$Tru, VN_INTVAR$Face_ID, mean))
)


# Combine data frames
Cor_A_T <- cbind.data.frame(Cor_A_T_1, Cor_A_T_2, Cor_A_T_3,Cor_A_T_4,Cor_A_T_5)

# Use rcorr from Hmisc
rcorr(as.matrix(Cor_A_T))

d <- rbind.data.frame(CZ_Orig_INTVAR, CZ_INTVAR_T, RSA_INTVAR_T, AUS_INTVAR_T, VN_INTVAR_T)
d_check <- rbind.data.frame(CZ_Orig_INTVAR[,c(1:9)], CZ_INTVAR, RSA_INTVAR, AUS_INTVAR, VN_INTVAR)

d$Sample_complete <- paste(d$Above_Below,d$Sample)

summary(as.factor(d$Sample_complete))
d <- d[d$Sample_complete!="mediocre AUS",] 
d <- d[d$Sample_complete!="mediocre VN",] 
summary(as.factor(d$Sample_complete))

# Load them all back again due to technical error spotted on 23rd of January...



write.csv2(d, file="Whole_Dataset_long_MORF_08_10.csv") # This time, it includes a variable that tells 
write.csv(d, file="1Whole_Dataset_long_MORF_08_10.csv") # This time, it includes a variable that tells 
# whether the rater should be kept - if she's close to the median in SMU frequency/intensity, she should be exluded...
...
# What I do want to end up with: 
# A csv that I can feed into R script "Multinom_eye_col...". 
# That is: 
# Initial columns for raters: Age, origin, SMU-i, etc. - one row one participant. I do not care about them much for this analysis
# Becuase I assign the eye colour as an aggregated conclusion based on the ratings: 

#-------------------------------------------------------------------------------
# EYECOLOR
# NNNN
# Preliminary: Obtaining the data on colour: 
CZ_CZ1 <- read.csv2("CZ_CZ_eye_face_SocMed_Checked.csv", T)

CZ_CZ1$CZ16F_eye_col_40
CZ_CZ1$CZ16M_eye_col_41
CZ_CZ1$CZ19F_eye_col_42
CZ_CZ1$CZ19M_eye_col_43

column_name1 <- "CZ16F_eye_col_40"
column_name2 <- "CZ16M_eye_col_41"
column_name3 <- "CZ19F_eye_col_42"
column_name4 <- "CZ19M_eye_col_43"

C1 <- match(column_name1, names(CZ_CZ1))
C2 <- match(column_name2, names(CZ_CZ1))
C3 <- match(column_name3, names(CZ_CZ1))
C4 <- match(column_name4, names(CZ_CZ1)) # IDK now why it works, but it does, see below - the correct collumns are concatenated 

# Subsetting just the rows that contain Eye colour ratings: 
CZRAT_List_EYES <- CZ_CZ1[,c(2,6,C1,C2,C3,C4,133)] # Note that Czechs rated both Eyes and Faces (not the same ppl., but people in the same survey...)
levels(as.factor(CZRAT_List_EYES$Task_Name_6))
CZRAT_List_EYES <- CZRAT_List_EYES[CZRAT_List_EYES$Task_Name_6=="CZ_16_F_eye"|CZRAT_List_EYES$Task_Name_6=="CZ_16_M_eye"|
                                     CZRAT_List_EYES$Task_Name_6=="CZ_19_F_eye"|CZRAT_List_EYES$Task_Name_6=="CZ_19_M_eye",]

View(CZRAT_List_EYES)

# Merging: 
CZRAT_List_EYES$CZ16F_eye_col_40[CZRAT_List_EYES$CZ16F_eye_col_40==""]<-NA
CZRAT_List_EYES$CZ16M_eye_col_41[CZRAT_List_EYES$CZ16M_eye_col_41==""]<-NA
CZRAT_List_EYES$CZ19F_eye_col_42[CZRAT_List_EYES$CZ19F_eye_col_42==""]<-NA
CZRAT_List_EYES$CZ19M_eye_col_43[CZRAT_List_EYES$CZ19M_eye_col_43==""]<-NA
...
CZRAT_List_EYES$EyeCol <- apply(CZRAT_List_EYES[,3:6], 1, get_non_na)
CZRAT_List_EYES$order <- seq(from=1, to=nrow(CZRAT_List_EYES))

# Which eye it is = which is the ID of the target's photo...
real_IDs <- read.csv2("CZ1619_Order_TRU.csv",T) # File of IDs.
real_IDs$ChctaSrt <- paste(real_IDs$Grupa_EYE,real_IDs$Trial_Id)
names(real_IDs)
names(real_IDs)[6] <- "Trial_Id_sup"

CZRAT_List_EYES$Trial_Id_sup <- paste(CZRAT_List_EYES$Task_Name_6,CZRAT_List_EYES$Trial_Id_2)

CZRAT_List_EYES$Trial_Id_sup

CZ_eyes_Merged <- merge(real_IDs, CZRAT_List_EYES, by="Trial_Id_sup")

CZ_eyes_Merged <- CZ_eyes_Merged[order(CZ_eyes_Merged$order),]

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(CZ_eyes_Merged)[13] <- "Subject_ID"
names(CZ_CZ_IP_good)[1] <- "Subject_ID"
CZ_CZ_IP_good$Subject_ID <- as.numeric(CZ_CZ_IP_good$Subject_ID)
summary(as.factor(CZ_eyes_Merged$Subject_ID))

# Add the rest of the variables -> 
#CZ_eyes_comp <- merge(CZ_eyes_Merged, CZ_CZ_IP_good, by="Subject_ID") 
#CZ_eyes_comp <- CZ_eyes_comp[order(CZ_eyes_comp$order),]

# Cut it to participants: 

?merge

# Ratings into columns so that one row = one face...
CZ_eyes_Merged$Real_ID <- paste(CZ_eyes_Merged$Set,CZ_eyes_Merged$Real_ID)

faces <- unique(CZ_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- CZ_eyes_Merged$EyeCol[CZ_eyes_Merged$Real_ID == faces[i]]
}

# Find the maximum length to pad shorter lists with NA
max_length <- max(sapply(face_list, length))

# Pad each list element with NA
face_list <- lapply(face_list, function(x) c(x, rep(NA, max_length - length(x))))

# Combine the list elements into a data frame
EyeWide_CZ <- as.data.frame(face_list)
...
#.-.-.-.-.-.-.-.-.
# VN 
VN_VN1 <- read.csv2("VN_VN_eye_face_SocMed_Checked_3.csv", T)

VN_VN1$CZ16F_eye_col
VN_VN1$CZ16M_eye_col
VN_VN1$CZ19F_eye_col
VN_VN1$CZ19M_eye_col

column_name1 <- "CZ16F_eye_col"
column_name2 <- "CZ16M_eye_col"
column_name3 <- "CZ19F_eye_col"
column_name4 <- "CZ19M_eye_col"

C1 <- match(column_name1, names(VN_VN1))
C2 <- match(column_name2, names(VN_VN1))
C3 <- match(column_name3, names(VN_VN1))
C4 <- match(column_name4, names(VN_VN1)) # IDK now why it works, but it does, see below - the correct collumns are concatenated 

# Subsetting just the rows that contain Eye colour ratings: 
VNRAT_List_EYES <- VN_VN1[,c(2,6,C1,C2,C3,C4,134)] # Note that Czechs rated both Eyes and Faces (not the same ppl., but people in the same survey...)
levels(as.factor(VNRAT_List_EYES$Task_Name))
VNRAT_List_EYES <- VNRAT_List_EYES[VNRAT_List_EYES$Task_Name=="CZ_16_F_eye"|VNRAT_List_EYES$Task_Name=="CZ_16_M_eye"|
                                     VNRAT_List_EYES$Task_Name=="CZ_19_F_eye"|VNRAT_List_EYES$Task_Name=="CZ_19_M_eye",]

View(VNRAT_List_EYES)

# Merging: 
VNRAT_List_EYES$CZ16F_eye_col[VNRAT_List_EYES$CZ16F_eye_col==""]<-NA
VNRAT_List_EYES$CZ16M_eye_col[VNRAT_List_EYES$CZ16M_eye_col==""]<-NA
VNRAT_List_EYES$CZ19F_eye_col[VNRAT_List_EYES$CZ19F_eye_col==""]<-NA
VNRAT_List_EYES$CZ19M_eye_col[VNRAT_List_EYES$CZ19M_eye_col==""]<-NA
...
# Apply the function across rows to create the new column
VNRAT_List_EYES$EyeCol <- apply(VNRAT_List_EYES[,3:6], 1, get_non_na)
VNRAT_List_EYES$order <- seq(from=1, to=nrow(VNRAT_List_EYES))
...
VNRAT_List_EYES$Trial_Id_sup <- paste(VNRAT_List_EYES$Task_Name,VNRAT_List_EYES$Trial_Id)

VNRAT_List_EYES$Trial_Id_sup

VN_eyes_Merged <- merge(real_IDs, VNRAT_List_EYES, by="Trial_Id_sup")

VN_eyes_Merged <- VN_eyes_Merged[order(VN_eyes_Merged$order),]

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(VN_eyes_Merged)[13] <- "Subject_ID"
names(VN_VN_IP_good)[1] <- "Subject_ID"
VN_VN_IP_good$Subject_ID <- as.numeric(VN_VN_IP_good$Subject_ID)
summary(as.factor(VN_eyes_Merged$Subject_ID))

# Cut it to participants: 

# Ratings into columns so that one row = one face...
VN_eyes_Merged$Real_ID <- paste(VN_eyes_Merged$Set,VN_eyes_Merged$Real_ID)

faces <- unique(VN_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- VN_eyes_Merged$EyeCol[VN_eyes_Merged$Real_ID == faces[i]]
}
...
# Combine the list elements into a data frame
EyeWide_VN <- as.data.frame(face_list)

print(EyeWide_VN)

VN_VN_P_ID2<-as.data.frame(VN_VN_P_ID)
VN_VN_P_ID2 <- data.frame(lapply(VN_VN_P_ID, as.character), stringsAsFactors=FALSE)

write.csv2(VN_VN_P_ID2, file="VN_faces_rating_16_09_24.csv")

#49 - co se zdaj v pohodě... na ksichty
#19 (cca, jinak 22-18) na voči
...
RSA_RSA1 <- read.csv("RJA_CZ_eye_SocMed_Checked.csv", T, sep=";")

RSA_RSA1$CZ16F_eye_col29
RSA_RSA1$CZ16M_eye_col30
RSA_RSA1$CZ19F_eye_col31
RSA_RSA1$CZ19M_eye_col32

column_name1 <- "CZ16F_eye_col29"
column_name2 <- "CZ16M_eye_col30"
column_name3 <- "CZ19F_eye_col31"
column_name4 <- "CZ19M_eye_col32"

C1 <- match(column_name1, names(RSA_RSA1))
C2 <- match(column_name2, names(RSA_RSA1))
C3 <- match(column_name3, names(RSA_RSA1))
C4 <- match(column_name4, names(RSA_RSA1))

# Subsetting just the rows that contain Eye colour ratings: 
RSARAT_List_EYES <- RSA_RSA1[,c(2,6,C1,C2,C3,C4,110)]
levels(as.factor(RSARAT_List_EYES$Task_Name6))
RSARAT_List_EYES <- RSARAT_List_EYES[RSARAT_List_EYES$Task_Name6=="CZ_16_F_eye"|RSARAT_List_EYES$Task_Name6=="CZ_16_M_eye"|
                                       RSARAT_List_EYES$Task_Name6=="CZ_19_F_eye"|RSARAT_List_EYES$Task_Name6=="CZ_19_M_eye",]

View(RSARAT_List_EYES)

# Merging: 
RSARAT_List_EYES$CZ16F_eye_col29[RSARAT_List_EYES$CZ16F_eye_col29==""]<-NA
RSARAT_List_EYES$CZ16M_eye_col30[RSARAT_List_EYES$CZ16M_eye_col30==""]<-NA
RSARAT_List_EYES$CZ19F_eye_col31[RSARAT_List_EYES$CZ19F_eye_col31==""]<-NA
RSARAT_List_EYES$CZ19M_eye_col32[RSARAT_List_EYES$CZ19M_eye_col32==""]<-NA

# Apply the function across rows to create the new column
RSARAT_List_EYES$EyeCol <- apply(RSARAT_List_EYES[,3:6], 1, get_non_na)
RSARAT_List_EYES$order <- seq(from=1, to=nrow(RSARAT_List_EYES))

# Which eye it is = which target's photo...
RSARAT_List_EYES$Trial_Id_sup <- paste(RSARAT_List_EYES$Task_Name6,RSARAT_List_EYES$Trial_Id2)

RSARAT_List_EYES$Trial_Id_sup

RSA_eyes_Merged <- merge(real_IDs, RSARAT_List_EYES, by="Trial_Id_sup")

RSA_eyes_Merged <- RSA_eyes_Merged[order(RSA_eyes_Merged$order),]

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(RSA_eyes_Merged)[13] <- "Subject_ID"
names(RSA_RSA_IP_good)[1] <- "Subject_ID"
RSA_RSA_IP_good$Subject_ID <- as.numeric(RSA_RSA_IP_good$Subject_ID)
summary(as.factor(RSA_eyes_Merged$Subject_ID))
...
# Cut it to participants: 
# Ratings into columns so that one row = one face...
RSA_eyes_Merged$Real_ID <- paste(RSA_eyes_Merged$Set,RSA_eyes_Merged$Real_ID)

faces <- unique(RSA_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- RSA_eyes_Merged$EyeCol[RSA_eyes_Merged$Real_ID == faces[i]]
}
...
# Combine the list elements into a data frame
EyeWide_RSA <- as.data.frame(face_list)

unique(RSA_eyes_Merged$Subject_ID)
...
AUS_AUS1 <- read.csv("Eye_CZ_PC_PROLIFIC_AUSTRALIA_NZ_Checked.csv", T, sep=";")

AUS_AUS1$CZ16F_eye_col_30
AUS_AUS1$CZ16M_eye_col_31
AUS_AUS1$CZ19F_eye_col_32
AUS_AUS1$CZ19M_eye_col_33

column_name1 <- "CZ16F_eye_col_30"
column_name2 <- "CZ16M_eye_col_31"
column_name3 <- "CZ19F_eye_col_32"
column_name4 <- "CZ19M_eye_col_33"

C1 <- match(column_name1, names(AUS_AUS1))
C2 <- match(column_name2, names(AUS_AUS1))
C3 <- match(column_name3, names(AUS_AUS1))
C4 <- match(column_name4, names(AUS_AUS1))

# Subsetting just the rows that contain Eye colour ratings: 
AUSRAT_List_EYES <- AUS_AUS1[,c(2,6,C1,C2,C3,C4,114)]
levels(as.factor(AUSRAT_List_EYES$Task_Name_6))
AUSRAT_List_EYES <- AUSRAT_List_EYES[AUSRAT_List_EYES$Task_Name_6=="CZ_16_F_eye"|AUSRAT_List_EYES$Task_Name_6=="CZ_16_M_eye"|
                                       AUSRAT_List_EYES$Task_Name_6=="CZ_19_F_eye"|AUSRAT_List_EYES$Task_Name_6=="CZ_19_M_eye",]

View(AUSRAT_List_EYES)

# Merging: 
AUSRAT_List_EYES$CZ16F_eye_col_30[AUSRAT_List_EYES$CZ16F_eye_col_30==""]<-NA
AUSRAT_List_EYES$CZ16M_eye_col_31[AUSRAT_List_EYES$CZ16M_eye_col_31==""]<-NA
AUSRAT_List_EYES$CZ19F_eye_col_32[AUSRAT_List_EYES$CZ19F_eye_col_32==""]<-NA
AUSRAT_List_EYES$CZ19M_eye_col_33[AUSRAT_List_EYES$CZ19M_eye_col_33==""]<-NA

# Apply the function across rows to create the new column
AUSRAT_List_EYES$EyeCol <- apply(AUSRAT_List_EYES[,3:6], 1, get_non_na)
AUSRAT_List_EYES$order <- seq(from=1, to=nrow(AUSRAT_List_EYES))

# Which eye it is = which target's photo...
AUSRAT_List_EYES$Trial_Id_sup <- paste(AUSRAT_List_EYES$Task_Name_6,AUSRAT_List_EYES$Trial_Id_2)

AUSRAT_List_EYES$Trial_Id_sup

AUS_eyes_Merged <- merge(real_IDs, AUSRAT_List_EYES, by="Trial_Id_sup")

AUS_eyes_Merged <- AUS_eyes_Merged[order(AUS_eyes_Merged$order),]

# POSEM

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(AUS_eyes_Merged)[13] <- "Subject_ID"
names(AUS_AUS_IP_good)[1] <- "Subject_ID"
AUS_AUS_IP_good$Subject_ID <- as.numeric(AUS_AUS_IP_good$Subject_ID)
summary(as.factor(AUS_eyes_Merged$Subject_ID))
...
# Cut it to participants: 
# Ratings into columns so that one row = one face...
AUS_eyes_Merged$Real_ID <- paste(AUS_eyes_Merged$Set,AUS_eyes_Merged$Real_ID)

faces <- unique(AUS_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- AUS_eyes_Merged$EyeCol[AUS_eyes_Merged$Real_ID == faces[i]]
}
...
# Combine the list elements into a data frame
EyeWide_AUS <- as.data.frame(face_list)

unique(AUS_eyes_Merged$Subject_ID)

# Print the final data frame
print(EyeWide_AUS)

write.csv2(EyeWide_AUS, file="AUS_wide_eyes.csv")

write.csv2(EyeWide_RSA, file="RSA_wide_eyes.csv")

write.csv2(EyeWide_CZ, file="CZ_wide_eyes.csv")

write.csv2(EyeWide_VN, file="VN_wide_eyes.csv")

write.csv2(EyeWide_COL, file="COL_wide_eyes.csv")


# To enter the eye project analyses...



#.-.-.-.-.-.-.-.-.
# COL 
COL_COL1 <- read.csv2("Colombia_FacesEyes_checked.csv", T)

COL_COL1$CZ16F_eye_col
COL_COL1$CZ16M_eye_col
COL_COL1$CZ19F_eye_col
COL_COL1$CZ19M_eye_col
...
C1 <- match(column_name1, names(COL_COL1))
C2 <- match(column_name2, names(COL_COL1))
C3 <- match(column_name3, names(COL_COL1))
C4 <- match(column_name4, names(COL_COL1)) # IDK now why it works, but it does, see below - the correct collumns are concatenated 

# Subsetting just the rows that contain Eye colour ratings: 
COLRAT_List_EYES <- COL_COL1[,c(2,6,C1,C2,C3,C4,134)] # Note that Czechs rated both Eyes and Faces (not the same ppl., but people in the same survey...)
levels(as.factor(COLRAT_List_EYES$Task_Name))
COLRAT_List_EYES <- COLRAT_List_EYES[COLRAT_List_EYES$Task_Name=="CZ_16_F_eye"|COLRAT_List_EYES$Task_Name=="CZ_16_M_eye"|
                                     COLRAT_List_EYES$Task_Name=="CZ_19_F_eye"|COLRAT_List_EYES$Task_Name=="CZ_19_M_eye",]

View(COLRAT_List_EYES)

# Merging: 
COLRAT_List_EYES$CZ16F_eye_col[COLRAT_List_EYES$CZ16F_eye_col==""]<-NA
COLRAT_List_EYES$CZ16M_eye_col[COLRAT_List_EYES$CZ16M_eye_col==""]<-NA
COLRAT_List_EYES$CZ19F_eye_col[COLRAT_List_EYES$CZ19F_eye_col==""]<-NA
COLRAT_List_EYES$CZ19M_eye_col[COLRAT_List_EYES$CZ19M_eye_col==""]<-NA
...
# Apply the function across rows to create the new column
COLRAT_List_EYES$EyeCol <- apply(COLRAT_List_EYES[,3:6], 1, get_non_na)
COLRAT_List_EYES$order <- seq(from=1, to=nrow(COLRAT_List_EYES))
...
COLRAT_List_EYES$Trial_Id_sup <- paste(COLRAT_List_EYES$Task_Name,COLRAT_List_EYES$Trial_Id)

COLRAT_List_EYES$Trial_Id_sup

COL_eyes_Merged <- merge(real_IDs, COLRAT_List_EYES, by="Trial_Id_sup")

COL_eyes_Merged <- COL_eyes_Merged[order(COL_eyes_Merged$order),]

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(COL_eyes_Merged)[13] <- "Subject_ID"
names(COL_COL_IP_good)[1] <- "Subject_ID"
COL_COL_IP_good$Subject_ID <- as.numeric(COL_COL_IP_good$Subject_ID)
summary(as.factor(COL_eyes_Merged$Subject_ID))

# Cut it to participants: 

# Ratings into columns so that one row = one face...
COL_eyes_Merged$Real_ID <- paste(COL_eyes_Merged$Set,COL_eyes_Merged$Real_ID)

faces <- unique(COL_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- COL_eyes_Merged$EyeCol[COL_eyes_Merged$Real_ID == faces[i]]
}
...
# Combine the list elements into a data frame
EyeWide_COL <- as.data.frame(face_list)

print(EyeWide_COL)

COL_COL_P_ID2<-as.data.frame(COL_COL_P_ID)
COL_COL_P_ID2 <- data.frame(lapply(COL_COL_P_ID, as.character), stringsAsFactors=FALSE)

write.csv2(COL_COL_P_ID2, file="COL_faces_rating_16_09_24.csv")


#.-.-.-.-.-.-.-.-.
# TURCJI 

# Je tu jakejsi bordel - nevrací to správnej dokument, ten širokej. 
# Ten, co se pak má uložit. A taky bacha, helper fajl obsahuje ID pro všechny
# Turky, co zadali ID (což je OK, protože to byla předposlední otázka, takže 
# Kdo došel až sem, je 100% použitelnej, jen na to bacha).

TUR_TUR1 <- read.csv("Turkey_25_12_24.csv", T)

TUR_TUR1$CZ16F_eye_col
TUR_TUR1$CZ16M_eye_col
TUR_TUR1$CZ19F_eye_col
TUR_TUR1$CZ19M_eye_col

column_name1 <- "CZ16F_eye_col"
column_name2 <- "CZ16M_eye_col"
column_name3 <- "CZ19F_eye_col"
column_name4 <- "CZ19M_eye_col"
column_name5 <- "FIELD1_StudentID"
column_name6 <- "FIELD2_Course_Code"

C1 <- match(column_name1, names(TUR_TUR1))
C2 <- match(column_name2, names(TUR_TUR1))
C3 <- match(column_name3, names(TUR_TUR1))
C4 <- match(column_name4, names(TUR_TUR1)) # IDK now why it works, but it does, see below - the correct collumns are concatenated 
C5 <- match(column_name5, names(TUR_TUR1))
C6 <- match(column_name6, names(TUR_TUR1))

# Subsetting just the rows that contain Eye colour ratings: 
TURRAT_List_EYES <- TUR_TUR1[,c(2,6,C1,C2,C3,C4,136, C5, C6)] # Note that Czechs rated both Eyes and Faces (not the same ppl., but people in the same survey...)

View(TURRAT_List_EYES)
TURRAT_List_EYES$FIELD2_Course_Code <- ifelse(TURRAT_List_EYES$FIELD2_Course_Code=="",NA,TURRAT_List_EYES$FIELD2_Course_Code)
TURRAT_List_EYES$FIELD1_StudentID <- ifelse(TURRAT_List_EYES$FIELD1_StudentID=="",NA,TURRAT_List_EYES$FIELD1_StudentID)

# Add Course ID and Student ID to each subject: 
Tur_help <- data.frame(ID=summary(as.factor(TURRAT_List_EYES$Exp_Subject_Id), maxsum=1e4))
Tur_help$Student_ID <- tapply(TURRAT_List_EYES$FIELD1_StudentID, TURRAT_List_EYES$Exp_Subject_Id, get_word2)
Tur_help$Course_Code <- tapply(TURRAT_List_EYES$FIELD2_Course_Code, TURRAT_List_EYES$Exp_Subject_Id, get_word2)

Tur_help <- Tur_help[!is.na(Tur_help$Course_Code),]

write.csv2(Tur_help2, file="Turkish_student_participants_faces_only.csv")
write.csv2(Tur_help, file="Turkish_student_participants.csv")

levels(as.factor(TURRAT_List_EYES$Task_Name))
TURRAT_List_EYES <- TURRAT_List_EYES[TURRAT_List_EYES$Task_Name=="CZ_16_F_eye"|TURRAT_List_EYES$Task_Name=="CZ_16_M_eye"|
                                     TURRAT_List_EYES$Task_Name=="CZ_19_F_eye"|TURRAT_List_EYES$Task_Name=="CZ_19_M_eye",]

# Merging: 
TURRAT_List_EYES$CZ16F_eye_col[TURRAT_List_EYES$CZ16F_eye_col==""]<-NA
TURRAT_List_EYES$CZ16M_eye_col[TURRAT_List_EYES$CZ16M_eye_col==""]<-NA
TURRAT_List_EYES$CZ19F_eye_col[TURRAT_List_EYES$CZ19F_eye_col==""]<-NA
TURRAT_List_EYES$CZ19M_eye_col[TURRAT_List_EYES$CZ19M_eye_col==""]<-NA
...
# Apply the function across rows to create the new column
TURRAT_List_EYES$EyeCol <- apply(TURRAT_List_EYES[,3:6], 1, get_non_na)
TURRAT_List_EYES$order <- seq(from=1, to=nrow(TURRAT_List_EYES))

length(summary(as.factor(TURRAT_List_EYES$Exp_Subject_Id)))
...
TURRAT_List_EYES$Trial_Id_sup <- paste(TURRAT_List_EYES$Task_Name,TURRAT_List_EYES$Trial_Id)

TURRAT_List_EYES$Trial_Id_sup

TUR_eyes_Merged <- merge(real_IDs, TURRAT_List_EYES, by="Trial_Id_sup")

TUR_eyes_Merged <- TUR_eyes_Merged[order(TUR_eyes_Merged$order),]

# Prepare (unite) names for merging with the funny data on use of the internet, etc...
names(TUR_eyes_Merged)[13] <- "Subject_ID"
names(TUR_TUR_IP_good)[1] <- "Subject_ID"
TUR_TUR_IP_good$Subject_ID <- as.numeric(TUR_TUR_IP_good$Subject_ID)
summary(as.factor(TUR_eyes_Merged$Subject_ID))

# Cut it to participants: 

# Ratings into columns so that one row = one face...
TUR_eyes_Merged$Real_ID <- paste(TUR_eyes_Merged$Set,TUR_eyes_Merged$Real_ID)

faces <- unique(TUR_eyes_Merged$Real_ID)

face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- TUR_eyes_Merged$EyeCol[TUR_eyes_Merged$Real_ID == faces[i]]
}
...
# Combine the list elements into a data frame
EyeWide_TUR <- as.data.frame(face_list)

print(EyeWide_TUR)

TUR_TUR_P_ID2<-as.data.frame(TUR_TUR_P_ID)
TUR_TUR_P_ID2 <- data.frame(lapply(TUR_TUR_P_ID, as.character), stringsAsFactors=FALSE)

write.csv2(TUR_TUR_P_ID2, file="TUR_faces_rating_16_09_24.csv")
...
# Mock data frame: 
Mock <- data.frame(VAR1 = sample(rep(paste("F", seq(1:10)),4)),
                   VAR2 = sample(c("Blue","Brown","Other"),40, replace = T)
                   )

Mock$ID <- seq_along(Mock$VAR1)

# Unique values of VAR1
faces <- unique(Mock$VAR1)

# Initialize an empty list to hold the values for each face
face_list <- setNames(vector("list", length(faces)), faces)

# Fill the list with VAR2 values according to VAR1
for (i in 1:length(faces)) {
  face_list[[i]] <- Mock$VAR2[Mock$VAR1 == faces[i]]
}
...
# Combine the list elements into a data frame
final_df <- as.data.frame(face_list)
